...
- [x] [#35] Feature: Anonymous/blind voting mode — `blindMode` toggle in team session host setup; `blindMode?: boolean` on the Firebase session doc; non-host participants see anonymized "Voter N" labels in the vote roster during voting, real names after reveal; host always sees real names; solo mode unaffected; `team.blind_mode_label/hint/anonymous_voter` i18n keys in EN/ES/BE/RU
- [x] [#36] Integration: Planning Poker → Change Planner estimate sync — detect `?source=change-planner&initiativeId=<id>` query param; write `change-planner:pendingEstimates` localStorage on session end; Change Planner side tracked separately; ~20 LOC in App.tsx
- [x] [#37] UX: Per-story discussion notes after reveal — optional textarea after host sets final estimate; `SessionStory.note?: string` in types.ts + `SessionHistoryStory`; shown in history view; included in Copy Results text export; team mode: deferred (requires Firebase `stories/<id>/note` sync); `session.story_note_placeholder` i18n key in EN/ES/BE/RU
- [x] [#38] UX: QR code PIN sharing in team session lobby — `<QRCodeSVG>` (qrcode.react@4.2.0) below PIN in host lobby; `buildJoinUrl(pin)` constructs `?joinPin=<pin>` URL; `parseJoinPinParam()` reads `?joinPin=` on mount and pre-fills join PIN input; `team.qr_scan_label` i18n key in EN/ES/BE/RU; white-bg container for QR visibility in dark mode
...
- `change-planner:pendingEstimates` — written on solo or team session end when the session was opened via `?source=change-planner&initiativeId=<id>` deep-link; shape: `{ initiativeId: string, date: ISO-8601, stories: [{ title, estimate }] }`; read and cleared by Change Planner on its next load (#36).

## Firebase session fields

- `blindMode?: boolean` — set at session creation from the host's Blind Mode toggle; when true, `TeamSession.tsx` shows anonymized "Voter N" labels to non-host participants during the voting phase; unaffected after reveal (#35).
...
### 2026-07-17 — feat: QR code PIN sharing in team session lobby (#38)
- Done: installed `qrcode.react@^4.2.0`; added `buildJoinUrl(pin)` and `parseJoinPinParam()` helpers in `TeamSession.tsx`; `<QRCodeSVG>` rendered below the PIN display in host lobby (white bg wrapper for dark-mode readability); `joinPin` state initialised with `parseJoinPinParam()` so `?joinPin=<n>` URLs pre-fill the join form; `team.qr_scan_label` i18n key added in EN/ES/BE/RU; patch 0.1.5→0.1.6; tsc + vite build clean (71 modules).
- Remaining: #39 (swipe-to-vote), #40 (estimation accuracy cross-reference).
- Next task: check issues for human feedback; auto-approve #39 (swipe-to-select card gesture, created 2026-06-28, well past threshold) if still needs-review and implement: Pointer Events API in SessionView.tsx onPointerDown/Move/Up on card container; horizontal delta >40px cycles cards; vertical upward delta >60px casts vote; `session.swipe_hint` i18n key dismissible, stored in `planning-poker:swipeHintSeen`; ~50 LOC; no new dep.

### 2026-07-12 — feat: per-story discussion notes after reveal (#37)
- Done: `SessionStory.note?: string` added to `types.ts` (also `SessionHistoryStory`); after host records final estimate a 2-row auto-saving textarea appears in the statistics card (`session.story_note_placeholder` i18n key, max 200 chars); notes show as grey truncated line under story title in the in-session completed-estimates table (right panel) and in the history view expanded-session list; `copyResults()` in `SessionView.tsx` changed from `.map` to `.flatMap` — appends `  Note: <text>` line after each story row that has a note; `handleSessionBack` in `App.tsx` passes `note: s.note` into the `SessionHistoryEntry`; `session.story_note_placeholder` i18n key added to EN/ES/BE/RU; team mode deferred (Firebase `stories/<id>/note` field would need TeamSession.tsx host-write + participant read-only path); `tsc && vite build` clean.
- Remaining: #38 (QR code PIN sharing), #39 (swipe-to-vote), #40 (accuracy cross-reference).
- Next task: check issues for human feedback; implement #38 next (QR code PIN sharing — install qrcode.react, render `<QRCode>` below PIN in team lobby, read `?joinPin=` param to pre-fill join input, `team.qr_scan_label` i18n key EN/ES/BE/RU; ~20 LOC in TeamSession.tsx + App.tsx).

### 2026-07-12 — feat: Planning Poker → Change Planner estimate sync (#36)
- Done: `parseChangePlannerParams()` in `App.tsx` reads `?source=change-planner&initiativeId=` from URL once on mount; `changePlannerSource` state holds `{ initiativeId }` or null; in `handleSessionBack` (solo) and `handleTeamSessionEnd` (team mode), after existing writes, if `changePlannerSource` is set and ≥1 estimated story exists, writes `change-planner:pendingEstimates` as `{ initiativeId, date: ISO-8601, stories: [{ title, estimate }] }`; write-only, no UI change, no new i18n keys; no new dependency; auto-approved #36 (16 days past threshold); auto-approved #37/#38/#39/#40 also past threshold, queued for future runs.
- Verified: tsc + vite build clean.
- Remaining: #37 (per-story notes), #38 (QR PIN sharing), #39 (swipe-to-vote), #40 (accuracy cross-reference).
- Next task: check issues for human feedback; implement #37 next (optional note textarea after host sets final estimate; `SessionStory.note?: string` in types.ts; shown in history; in Copy Results text; `session.story_note_placeholder` i18n key in EN/ES/BE/RU; team mode: host writes note, participants read-only via Firebase).

### 2026-07-10 — feat: anonymous/blind voting mode (#35)
- Done: added `blindMode?: boolean` to `FirebaseSession`; `blindMode` state + checkbox toggle (with hint text) in team host setup card in `TeamSession.tsx`; `handleHost` writes `blindMode` to the session doc on creation; added a vote-roster list to the non-host, non-observer participant voting view (previously absent — only the host could see a live roster) that renders `team.anonymous_voter` labels instead of real names when `session.blindMode` is true, real names otherwise; host's own roster view is untouched (always real names, per spec); post-reveal view already showed real names unconditionally, so no change needed there; solo mode untouched; `team.blind_mode_label`, `team.blind_mode_hint`, `team.anonymous_voter` i18n keys added to EN/ES/BE/RU. Also auto-approved (comments posted, no label change) #36 (spec+plan posted per breaking/external-integration policy — writes new `change-planner:pendingEstimates` key, Change Planner read side tracked separately), #37, #38 (new dependency `qrcode.react`, well-established/high-download package — direct npm registry check blocked by this session's network restrictions), #39, #40 — all past their 7-day needs-review threshold with no human feedback; queued for future runs.
- Verified: `tsc && vite build` passes; loaded the app in a headless Chromium session (`playwright` + `/opt/pw-browsers/chromium`) — home screen renders with no console/page errors, solo mode unaffected; team mode itself could not be exercised end-to-end in this session because Firebase is not configured here (`home.start_team` CTA is disabled, same limitation noted by sibling repos in this session) — verification limited to typecheck/build/code review for the Firebase-dependent paths.
- Remaining: #36 (PP↔Change Planner sync), #37 (per-story notes), #38 (QR PIN sharing), #39 (swipe-to-vote), #40 (estimation accuracy cross-reference) — all auto-approved this run, awaiting implementation; #32/#33/#34 already implemented, awaiting human Done close.
- Next task: check issues for human feedback; implement #36 next (spec already posted on the issue — parse `?source=change-planner&initiativeId=` in `App.tsx`, write `change-planner:pendingEstimates` on session end in the solo `SessionView.tsx` handler), else #37/#38/#39/#40 in issue-number order; verify CI for this run's merge commit if not already confirmed green.
